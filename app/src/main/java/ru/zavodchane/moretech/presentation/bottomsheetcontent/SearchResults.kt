...
fun SearchResults() { // TODO: Докинь сюда параметр списка с результатом
    LazyColumn(
        ...
        items(4) { // TODO: и этот параметр сюда закинь
            ResultItem(address = "г. Москва, ул. Нижняя Красносельская, д. 45/17", distance = 1220, modifier = Modifier.padding(bottom = 5.dp
            ))
        }
    }
}
...
fun DistanceText(text: String, modifier: Modifier = Modifier) {
    Text(
        ...
        modifier = modifier.padding(bottom = 5.dp) // id = R.dimen.padding_small
    )
}
